...
library BalanceLib {
    function slot(BalanceMap storage self, address account) internal pure returns (uint256 _slot) {
    ...
    function set(BalanceMap storage self, address account, uint256 _balance) internal {
        uint256 _slot = self.slot(account);
        assembly ("memory-safe") {
